        ...
        for i = 1:length(initGuessType_list)        % for loop 3: initial condition
            ...
            for k = 1:length(solverIndex_list)        % for loop 4: iterative method
                ...
                switch solverIndex
                    case 0
                        algName = 'backslash';
                        [u1,t1,iter1,res1] = solvers(A, f1vec, solverIndex, timingBoolean);   % (M-1)(N-1) vector and avg run time
                        [u2,t2,iter2,res2] = solvers(A, f2vec, solverIndex, timingBoolean);
                        figure
                        subplot(2,1,1)
                        plot(res1,'*');
                        title(sprintf('Eqn1 [%s]: M=%d, N=%d, iter=%d, time=%0.4f',algName,M,N,iter1,t1));
                        ylim([0 0.1]); xlabel('iteration'); ylabel('norm(res)');
                        subplot(2,1,2)
                        plot(res2,'*');
                        title(sprintf('Eqn2 [%s]: M=%d, N=%d, iter=%d, time=%0.4f',algName,M,N,iter2,t2));
                        ylim([0 0.01]); xlabel('iteration'); ylabel('norm(res)');
                        if savePlotBoolean
                    ...
                    case 1
                        for l = 1:length(relaxation_list)-1        % for loop 5: relaxation
                            ...
                            subplot(2,1,1)
                            plot(res1);
                            title(sprintf('Eqn1 [%s, u%d]: M=%d, N=%d, iter=%d, time=%0.4f',algName,initGuessType,M,N,iter1,t1));
                            ylim([0 0.1]); xlabel('iteration'); ylabel('norm(res)');
                            subplot(2,1,2)
                            plot(res2);
                            title(sprintf('Eqn2 [%s, u%d]: M=%d, N=%d, iter=%d, time=%0.4f',algName,initGuessType,M,N,iter2,t2));
                            ylim([0 0.01]); xlabel('iteration'); ylabel('norm(res)');
                            if savePlotBoolean
